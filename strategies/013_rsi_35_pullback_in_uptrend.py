"""Strategy 013 — RSI 35 Pullback in Uptrend.

Run:  python strategies/013_rsi_35_pullback_in_uptrend.py
"""
...
def signal_mask(d):
    ...
    rsi_condition = d['rsi'] < 35
    uptrend = d['close'] > d['sma_200']
    
    return (rsi_condition & uptrend).fillna(False).values
...
def run():
    ...
    for ticker, df in panel.items():
        d = df.dropna(subset=["rsi", "close", "sma_200"]).reset_index(drop=True)
        if len(d) < 300:
            continue
    ...
    print(report("RSI 35 Pullback in Uptrend", strat, ctrl))
