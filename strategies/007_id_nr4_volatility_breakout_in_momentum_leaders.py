"""Strategy 007 — Inside-Day Narrow Range 4 (ID/NR4) Volatility Breakout in Momentum Leaders.

Hypothesis:
In liquid Indian equities (turnover >= ₹25cr/day), when an intermediate relative strength leader
(60-day momentum > 15%, Close > SMA 50, SMA 50 > SMA 200) prints an Inside-Day Narrowest Range of 4
sessions (ID/NR4)—representing complete order-flow equilibrium and volatility compression—a breakout
above the previous day's high accompanied by above-average volume (> 1.2x 20-day median) signals
the start of an institutional trend expansion, producing a positive swing edge over 6–10 trading days.

Run:  python strategies/007_id_nr4_volatility_breakout_in_momentum_leaders.py
"""
...
def add_id_nr4_features(d):
    """Add candle range, inside day, and NR4 features."""
    d['bar_range'] = d['high'] - d['low']
    d['nr4'] = d['bar_range'] == d['bar_range'].rolling(4).min()
    d['inside_day'] = (d['high'] < d['high'].shift(1)) & (d['low'] > d['low'].shift(1))
    d['id_nr4'] = d['inside_day'] & d['nr4']
    d['vol_med_20'] = d['volume'].rolling(20).median()
    d['vol_ratio'] = d['volume'] / d['vol_med_20']
    return d


def signal_mask(d, mom_th=MOM_THRESHOLD, vol_mult=VOL_MULT):
    """ID/NR4 breakout on above-average volume in momentum leader."""
    setup = d['id_nr4'].shift(1).fillna(False)
    breakout = (d['close'] > d['high'].shift(1))
    vol_confirm = (d['vol_ratio'] > vol_mult)
    trend = (d['momentum_60d'] > mom_th) & (d['close'] > d['sma_50']) & (d['sma_50'] > d['sma_200'])
    return (setup & breakout & vol_confirm & trend).fillna(False).values


def run():
    print(f"Loading 5-year panel for {len(UNIVERSE)} liquid NSE stocks...", flush=True)
    ...
    for ticker, df in panel.items():
        d = df.dropna(subset=['rsi', 'atr', 'close', 'sma_200', 'sma_50', 'turnover_60d', 'momentum_60d']).reset_index(drop=True)
        if len(d) >= 300:
            d = add_id_nr4_features(d)
            valid_dfs.append(d)
    ...
    print(report(f"Strategy 007: ID/NR4 Volatility Breakout ({HORIZON}d hold, costs charged)", strat, ctrl, holding_days=HORIZON), flush=True)

    dc = day_clustered_edge(strat, ctrl)
    print("\n--- DAY-CLUSTERED SINGLE-DRAW HEADLINE ---", flush=True)
    if dc:
        print(f"  z_paired = {dc['z_paired']:.2f}, paired_days = {dc['n_paired_days']}, day_edge = {dc['day_edge']:+.3f}%", flush=True)

    # 2. Stable 20-seed control
    ...
    print("\n--- STABLE 20-SEED DAY-CLUSTERED CONTROL ---", flush=True)
    if stable:
        print(f"  mean_z: {stable['mean_z']:.2f} | min_z: {stable['min_z']:.2f} | max_z: {stable['max_z']:.2f} | pass_rate: {stable['pass_rate']*100:.1f}% (n_seeds={stable['n_seeds']})", flush=True)

    # 3. Walk-forward validation (4 purged and embargoed folds)
    sample_len = len(valid_dfs[0])
    splits = list(walk_forward_splits(sample_len, n_splits=4, horizon_days=HORIZON))
    print(f"\n--- WALK-FORWARD VALIDATION ({len(splits)} purged folds) ---", flush=True)
    for fold_idx, ((tr0, tr1), (te0, te1)) in enumerate(splits, 1):
        ...
        tr_cnt = len(f_strat)
        print(f"  Fold {fold_idx} ({te0}:{te1}): trades={tr_cnt:3d}, net_edge={net_edge:+.2f}%, z_paired={zp:+.2f}, day_edge={de:+.2f}%", flush=True)

    # 4. Execution Timing Test (Next-Open fill vs Same-Close fill)
    print("\n--- EXECUTION TIMING TEST ---", flush=True)
    s_open, c_open = [], []
    for d in valid_dfs:
        liq = (d['turnover_60d'] >= MIN_TURNOVER).fillna(False).values
        sig = signal_mask(d) & liq
    ...
    mean_z_op = st_open['mean_z'] if st_open else 0.0
    pass_op = st_open['pass_rate'] * 100 if st_open else 0.0
    print(f"  Next-Open Fill: trades={len(s_open)}, Net={np.mean([t['net_pct'] for t in s_open]):+.2f}%, Edge={res_open['edge']:+.2f}%, mean_z={mean_z_op:.2f}, pass_rate={pass_op:.0f}%", flush=True)

    # 5. Parameter Ladder Sweep & Deflated Sharpe
    print("\n--- PARAMETER LADDER (Momentum Threshold Gradient) ---", flush=True)
    moms = [5.0, 10.0, 15.0, 20.0, 25.0, 30.0]
    all_srs = []
    trial_rets = []
    print(f"{'Mom_60 Threshold':<20} {'Trades':<8} {'Net Avg%':<10} {'z_naive':<10} {'z_paired':<10} {'mean_z':<10} {'pass%':<8}", flush=True)
    for m in moms:
        s_tr = []
        for d in valid_dfs:
            liq = (d['turnover_60d'] >= MIN_TURNOVER).fillna(False).values
            sig = signal_mask(d, mom_th=m) & liq
        ...
        res_m = edge_vs_control(s_rets, [t['net_pct'] for t in ctrl])
        dc_m = day_clustered_edge(s_tr, ctrl)
        ...
        st_m = stable_day_clustered_z(s_tr, cf_m, n_seeds=10)
        n_t = len(s_tr)
        net_avg = np.mean(s_rets) if s_rets else 0.0
        z_n = res_m['z'] if res_m else 0.0
        z_p = dc_m['z_paired'] if dc_m else 0.0
        m_z = st_m['mean_z'] if st_m else 0.0
        p_r = st_m['pass_rate'] * 100 if st_m else 0.0
        print(f"{m:<20.1f} {n_t:<8d} {net_avg:<+10.2f} {z_n:<10.2f} {z_p:<10.2f} {m_z:<10.2f} {p_r:<8.0f}", flush=True)

    eff = effective_trials(trial_rets)
    dsr = deflated_sharpe(max(all_srs), all_srs, n_obs=len(strat))
    print(f"\nEffective trials: {eff:.2f} (from {len(all_srs)} grid points)", flush=True)
    if dsr:
        print(f"Deflated Sharpe: DSR = {dsr['dsr']:.4f} (Observed SR: {dsr['observed_sr']:.2f}, Noise ceiling: {dsr['noise_ceiling_sr']:.2f})", flush=True)

    # 6. Cap-Tier Breakdown
    print("\n--- CAP-TIER BREAKDOWN ---", flush=True)
    for group_name, grp_dfs in [("Large Caps (Nifty 50)", large_dfs), ("Mid & Small Caps", midsmall_dfs)]:
        ...
        print(f"  {group_name:<25}: Trades={n_t:3d}, Net={net_avg:+.2f}%, Edge={edge_g:+.2f}%, Single z_p={zp_g:+.2f}, Stable mean_z={mz_g:+.2f} (pass {pr_g:.0f}%)", flush=True)

    # 7. Final verdict guide
    print("\n--- VERDICT ASSESSMENT ---", flush=True)
    if stable and stable['mean_z'] >= 2.0 and stable['pass_rate'] >= 0.8:
        print("  -> Meets pooled bar (still verify subgroup & walk-forward).", flush=True)
    else:
        print(f"  -> Does NOT clear the bar: stable mean_z {stable['mean_z'] if stable else 0.0:.2f} < 2.0. REJECT.", flush=True)
