...
Hypothesis:
When a liquid mid/small cap stock transitions into a full multi-timeframe moving average ribbon alignment
(EMA 10 > SMA 20 > SMA 50 > SMA 200), it signals the initiation of coordinated institutional accumulation
across short-, medium-, and long-term trend horizons, generating persistent positive drift over a 15-21 session
holding horizon.

Rules and kill criteria are pre-registered in strategies/030_moving_average_ribbon_alignment_expansion_in_liquid_mid_caps.md.
Run:  python strategies/030_moving_average_ribbon_alignment_expansion_in_liquid_mid_caps.py
"""
...
def prepare_panel(panel):
    ...
    for t, df in panel.items():
        ...
        if len(d) < 260:
            continue
        d["ticker"] = t
        ribbon = (d["ema_10"] > d["sma_20"]) & (d["sma_20"] > d["sma_50"]) & (d["sma_50"] > d["sma_200"])
        ribbon_prev = ribbon.shift(1).fillna(False)
        d["ribbon_entry"] = ribbon & (~ribbon_prev)
        d["liq"] = (d["turnover_60d"] >= MIN_TURNOVER).fillna(False)
        stocks[t] = d
    return stocks


def run_set(sub_stocks, horizon=HORIZON, next_open=False, seeds=20):
    strat = []
    for t, d in sub_stocks.items():
        sig = d["ribbon_entry"].values & d["liq"].values
        if next_open:
            sig = np.roll(sig, 1)
            sig[0] = False
...
def simulate_portfolio(sub_stocks, horizon=HORIZON, cost_mult=1.0, max_slots=20):
    all_events = []
    for t, d in sub_stocks.items():
        sig = d["ribbon_entry"].values & d["liq"].values
    ...
    dates_list = sorted(list({d for s in sub_stocks.values() for d in s["date"]}))
        ...
        if dt in events_by_date.groups:
            ...
            for _, row in day_evs.iterrows():
                ...
                if empty_slot is not None:
                    ...
                    exit_d = dates_list[min(curr_idx + row["held"], len(dates_list) - 1)]
                    slots[empty_slot] = {
                        "ticker": row["ticker"],
                        "nav_at_entry": nav,
                        "exit_date": exit_d,
    ...
    nav_series = pd.Series(daily_values, index=dates_list)
...
def main():
    ...
    print("=== 1. Headline Engine Run (Full 10y Pooled, H=21d) ===", flush=True)
    res = run_set(stocks, horizon=21, seeds=20)
    if res:
    ...
    res_B = run_set({t: stocks[t] for t in half_B}, horizon=21, seeds=10)
    if res_B:
        print(f"Holdout Half B : N={res_B['n']:4d} Days={res_B['days']:4d} Net={res_B['net']:+.3f}% Day_edge={res_B['day_edge']:+.3f}% Mean_z={res_B['mean_z']:+5.2f} (pass {res_B['pass_rate']*100:.0f}%)", flush=True)

    res_pre = run_set({t: stocks[t] for t in pre_2017}, horizon=21, seeds=10)
    if res_pre:
        print(f"Pre-2017 Only  : N={res_pre['n']:4d} Days={res_pre['days']:4d} Net={res_pre['net']:+.3f}% Day_edge={res_pre['day_edge']:+.3f}% Mean_z={res_pre['mean_z']:+5.2f} (pass {res_pre['pass_rate']*100:.0f}%)\n", flush=True)

    print("=== 3. Execution Fragility: Next-Open Fill ===", flush=True)
    res_next = run_set(stocks, horizon=21, next_open=True, seeds=10)
    if res_next:
        print(f"Next-Open Entry: N={res_next['n']:4d} Days={res_next['days']:4d} Net={res_next['net']:+.3f}% Day_edge={res_next['day_edge']:+.3f}% Mean_z={res_next['mean_z']:+5.2f} (pass {res_next['pass_rate']*100:.0f}%)\n", flush=True)

    print("=== 4. Walk-Forward 5 Folds ===", flush=True)
    for k, (tr, te) in enumerate(walk_forward_splits(len(dates), n_splits=5, horizon_days=21)):
        if len(te) == 0:
            continue
        f_lo, f_hi = dates[te[0]], dates[te[-1]]
        f_sub = {t: d[(d["date"] >= f_lo) & (d["date"] <= f_hi)].reset_index(drop=True) for t, d in stocks.items()}
        rf = run_set(f_sub, horizon=21, seeds=10)
        if rf:
    ...
    print("\n=== 5. Portfolio Tool Test (20 Slots) ===", flush=True)
    p_10 = simulate_portfolio(stocks, horizon=21, cost_mult=1.0)
    p_15 = simulate_portfolio(stocks, horizon=21, cost_mult=1.5)
    p_20 = simulate_portfolio(stocks, horizon=21, cost_mult=2.0)
    if p_10:
