...
Holds for 10 trading sessions (2 calendar weeks).

Tested on 10-year master panel (2016–2026).
Run:  python strategies/033_sortino_momentum_top_decile_10_session_swing_in_mid_small_caps.py
"""
...
def load_panel():
    ...
    for t, d in prepped.items():
        ...
        d["sortino252_rank"] = sub_d["sortino252_rank"].reindex(idx).values
        d["mom_incumbent_rank"] = sub_d["mom_incumbent_rank"].reindex(idx).values
...
def run_portfolio_simulation(sub, horizon=HORIZON, n_slots=20, cost_mult=1.0):
...
def main():
    ...
    print(f"Loaded {len(panel)} tickers. Horizon = {HORIZON} sessions (2 calendar weeks).\n")
    
    print("=== 1. HEADLINE SIGNIFICANCE vs CONTROLS ===")
    strat, cf = run_trades_set(full, horizon=HORIZON)
    report_out = report("Strategy 033 — Sortino Momentum 10-Session Swing", strat, cf(42, "random"))
    print(report_out)
    print()
    ...
    print("\n=== 4. CHRONOLOGICAL WALK-FORWARD FOLDS (PURGED & EMBARGOED) ===")
    ...
    print("\n=== 5. PORTFOLIO SIMULATION STRESS TEST ===")
    for cm in [1.0, 1.5, 2.0]:
        p = run_portfolio_simulation(full, horizon=HORIZON, n_slots=20, cost_mult=cm)
        print(f"Cost {cm:3.1f}x ({cm*0.50:4.2f}% RT) | Strategy CAGR: {p['cagr']:+5.2f}%, Sharpe: {p['sharpe']:.2f}, MaxDD: {p['max_dd']:+5.2f}%, Trades: {p['trades']}")
