...
Buys liquid NSE mid/small cap stocks in an established Golden-Cross structural bull regime
(SMA_50 > SMA_200 and Close > SMA_200) experiencing acute oversold dislocation
(Wilder RSI(14) < 30).

Holds for 8 trading sessions (with 6d and 10d reported).
Tested on 10-year master panel (2016–2026).
Run:  python strategies/035_capitulation_washout_in_structural_uptrend_in_liquid_mid_small_caps.py
"""
...
MIN_TURNOVER = 25e7
HORIZON = 8 # 8 trading sessions baseline swing hold
...
def run():
    print("=" * 80)
    print("STRATEGY 035: Golden Cross Oversold Rebound in Liquid Mid-Small Caps (8d Swing)")
    print("=" * 80)
    ...
    # 1. Baseline Strategy Trades (Horizon = 8d, Close > SMA_200, SMA_50 > SMA_200, RSI < 30)
    strat_trades = []
    strat_no = []
    for t, d in stocks.items():
        liq = d["liq"].values
        sig = (d["rsi"] < 30) & d["uptrend_200"] & d["golden_cross"] & liq
    ...
    # 2. Pre-generate 20 Random Control Seeds (Pooled, Next-Open)
    ...
    print("\n[3] Head-to-Head vs Incumbent RSI<30 Baseline (METHODOLOGY §10):")
    inc_trades = []
    for t, d in stocks.items():
        liq = d["liq"].values
        sig = (d["rsi"] < 30) & liq
    ...
    print(f"    Post-2017 (n={len(strat_post17):3d}): stable mean z = {np.mean(zs_post17):+.2f}")
    ...
    # 8. Contrast with Large Caps (Nifty 50)
    print("\n[8] Large-Cap Failure Contrast (Nifty 50 Constituents):")
    strat_large = []
    ctrl_large = []
    rng_l = np.random.default_rng(42)
    for t, d in large_caps.items():
        liq = (d['turnover_60d'] >= MIN_TURNOVER).fillna(False)
        sig = (d["rsi"] < 30) & d["uptrend_200"] & d["golden_cross"] & liq
    ...
    large_net = np.mean([tr["net_pct"] for tr in strat_large]) if strat_large else 0
    ...
    print("\n" + "=" * 80)
    print(f"FINAL SUMMARY: Stable Mean z = {mean_z_rnd:+.2f} vs Rnd, {dc_inc['z_paired']:+.2f} vs Incumbent RSI<30")
    print("=" * 80)
